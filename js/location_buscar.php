...
class location extends dbconfig {
 ...
 // Fetch all countries list
  ...
  public static function getEstados($paisId) {
     try {
       $query = "SELECT id, name FROM states WHERE country_id=".$paisId;
       $result = dbconfig::run($query);
       if(!$result) {
 ...
 // Fetch all cities list by state id
  public static function getCiudades($estadoId) {
     try {
       $query = "SELECT id, name FROM cities WHERE state_id=".$estadoId;
       $result = dbconfig::run($query);
       if(!$result) {
